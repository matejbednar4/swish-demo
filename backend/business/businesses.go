...
func GetBusinesses(c *gin.Context, db *sql.DB) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&business.Id, &business.Email, &business.Password, &business.Name, &business.Type, &business.Location)
		if err != nil {
...
func GetBusinessById(c *gin.Context, db *sql.DB) {
	...
	err := db.QueryRow("SELECT * FROM businesses WHERE id=?", id).Scan(&business.Id, &business.Email, &business.Password, &business.Name, &business.Type, &business.Location)
...
func UpdateBusiness(c *gin.Context, db *sql.DB) {
	...
	result, err := db.Exec("UPDATE businesses SET name=?, type=?, location=? WHERE id=?", passedData.Name, passedData.Type, passedData.Location, id)
	if err != nil {
